...
export const adminBlockChannelViewCallback = async ({
  ...
  try {
    ...
    const selected =
      (view.state.values.restrictions_block?.restrictions_checkboxes?.selected_options as
    ...
    try {
      ...
      channelName = (info.channel as Record<string, unknown>)?.name as string | undefined;
    } catch {
      // Name is optional
    }

    blockChannel(channelId, orgId, { channelName, blockRead, blockWrite, blockedBy });
