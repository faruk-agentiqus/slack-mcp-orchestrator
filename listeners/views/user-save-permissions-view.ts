...
import { setUserOverrides, PERMISSION_KEYS, type PermissionMap } from '../../src/permissions/engine.js';
...
export const userSavePermissionsViewCallback = async ({
  ...
  try {
    const { userId, orgId } = JSON.parse(view.private_metadata) as { userId: string; orgId: string };

    const selected =
      (view.state.values.permissions_block?.permissions_checkboxes?.selected_options as
