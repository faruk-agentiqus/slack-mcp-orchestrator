...
export function blockChannel(
  channelId: string,
  orgId: string,
  opts: { channelName?: string; blockRead?: boolean; blockWrite?: boolean; blockedBy: string },
): void {
...
export function unblockChannel(channelId: string, orgId: string): void {
  const db = getDatabase();
  db.prepare('DELETE FROM channel_blocklist WHERE channel_id = ? AND org_id = ?').run(channelId, orgId);
}
...
export function getBlockedChannels(orgId: string): BlockedChannel[] {
  const db = getDatabase();
  const rows = db
    .prepare('SELECT channel_id, channel_name, block_read, block_write, blocked_by, created_at FROM channel_blocklist WHERE org_id = ?')
    .all(orgId) as Array<{
      channel_id: string;
...
export function getChannelBlock(channelId: string, orgId: string): BlockedChannel | null {
  const db = getDatabase();
  const row = db
    .prepare('SELECT channel_id, channel_name, block_read, block_write, blocked_by, created_at FROM channel_blocklist WHERE channel_id = ? AND org_id = ?')
